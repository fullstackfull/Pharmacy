...
class IntegrationsPanel implements Panel
{
    ...
    private const CATALOGUE = [
        [
            ...
            'category' => 'payments',
            'detail' => '14 gateway controllers: Bkash, FlutterwaveV3, LiqPay, MercadoPago, Paymera, Paymob, PayPal, Paystack, PayTabs, Paytm, RazorPay, SenangPay, SslCommerz, Stripe.',
            'client' => 'raw curl_exec in 13 of them; Http::withBasicAuth in PaymeraController only',
            'seam' => 'app/Http/Controllers/Payment_Methods/*Controller.php (PaymeraController.php:88,129 is the one Http:: caller)',
            'recorder' => 'The 13 curl gateways are invisible to an Http:: middleware. Either port them to Http::, or wrap each curl_exec in a DependencyRecorder::measure(service, operation, callable) helper.',
        ...
        [
            ...
            'category' => 'payments',
            'detail' => 'A second, older code path to the same gateway, separate from the controller above.',
            'client' => 'raw curl_exec',
            'seam' => 'app/Library/sslcommerz/AbstractSslCommerz.php:47,66 and SslCommerzNotification.php:60,73',
            'recorder' => 'Same wrapper as the curl gateways. Listed separately because porting the controller would not cover it.',
        ...
        [
            ...
            'name' => 'SMS providers',
            'category' => 'messaging',
            'detail' => 'Twilio, Nexmo, 2Factor, MSG91, Releans, AlphanetSMS, plus eight more configured through addon_settings. Every one returns the literal string "error" or "not_found" on failure and persists nothing.',
            'client' => 'Twilio SDK (new Client) plus raw curl_exec for the rest',
            'seam' => 'app/Utils/SMSModule.php:62 (Twilio SDK), :95, :128, :161, :198, :227 (curl)',
            'recorder' => 'Neither the SDK nor curl_exec goes through Laravel\'s HTTP client, so the middleware misses all of them. Wrap SMSModule::send() itself — one seam covers every provider.',
        ...
        [
            ...
            'category' => 'messaging',
            'detail' => 'Every order, chat and status push, plus the OAuth token exchange that authorises them.',
            'client' => 'Http::withHeaders / Http::asForm',
            'seam' => 'app/Traits/PushNotificationTrait.php:495 (send) and :523 (token)',
            'recorder' => 'Caught by one Http::globalMiddleware. Worth its own service key rather than folding into "google", because a token failure and a send failure are different outages.',
        ...
        [
            ...
            'category' => 'maps',
            'detail' => 'Address autocomplete, delivery distance and reverse geocoding — on the checkout path, so its latency is customer-visible.',
            'client' => 'Http::',
        ...
        [
            ...
            'category' => 'platform',
            'detail' => 'Header probes and downloads of files held on another host, from the file manager and the helper layer.',
            'client' => 'raw curl_exec',
            'seam' => 'app/Utils/Helpers.php:500 and app/Utils/FileManagerLogic.php:23',
            'recorder' => 'Not caught by the middleware. Lowest priority of the thirteen: it is neither on the checkout path nor customer-visible.',
        ...
        return [
            ...
            'fields' => [
                'dependency_call_recorder' => Metric::notConfigured(
                    source: self::SOURCE,
                    remedy: self::PRODUCER_REMEDY,
                    note: 'The table, ' . self::WRITER . ', the rollup, the retention plan and five Overview cards all read and write this and are waiting on one registration. Http:: callers would be covered at once; raw curl_exec, the Twilio SDK, the OpenAI SDK and Mail:: each need their own wrapper.',
                ),
