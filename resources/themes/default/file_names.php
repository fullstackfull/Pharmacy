<?php

return [
    'home' => 'web-views.home',
    'product_search_result' => 'web-views.partials._search-result',
    'user_profile' => '',
    'account_address_add' => '',
    'account_address_edit' => 'web-views.users-profile.account-address-edit',
    'account_orders' => 'web-views.users-profile.account-orders',
    'user_account' => 'web-views.users-profile.user-account',
    'order_invoice' => 'web-views.order.invoice',
    'account_order_details' => 'web-views.users-profile.account-order-details',
    'refund_details' => 'web-views.users-profile.refund-details',
    'seller_info' => 'web-views.users-profile.account-details.seller-info',
    'delivery_man_info' => 'web-views.users-profile.account-details.delivery-man-info',
    'order_details_review' => 'web-views.users-profile.account-details.reviews',
    'track_order_wise_result' => 'web-views.users-profile.account-details.track-order',
    'account_wishlist' => 'web-views.users-profile.account-wishlist',
    'user_restock_requests' => 'web-views.users-profile.account-restock-requests',
    'user_wallet' => 'web-views.users-profile.user-wallet',
    'user_loyalty' => 'web-views.users-profile.user-loyalty',
    'account_tickets' => 'web-views.users-profile.account-tickets',
    'ticket_view' => 'web-views.users-profile.ticket-view',
    'user_inbox' => 'web-views.users-profile.inbox.index',
    'user_inbox_message' => 'web-views.users-profile.inbox.messages',
    'refer_earn' => 'web-views.users-profile.refer-earn',
    'user_coupons' => 'web-views.users-profile.coupons',
    'tracking-page' => 'web-views.order.tracking-page',
    'track_order' => 'web-views.order.tracking',
    'account_compare_list' => '',
    'account_wishlist_partials' => 'web-views.partials._wish-list-data',
    'seller_registration' => 'web-views.seller-view.auth.register',
    'all_stores_page' => 'web-views.seller-view.sellers',
    'customer_auth_verify' => 'web-views.customer-views.auth.verify',
    'customer_auth_verify_otp_login' => 'web-views.customer-views.auth._otp-login-verify',
    'customer_auth_verify_otp_update_info' => 'web-views.customer-views.auth._otp-update-info',
    'customer_auth_verify_firebase_auth_verify' => 'web-views.customer-views.auth._firebase-login-verify',
    'products_details' => 'web-views.products.details',
    'products_cart_partials' => 'layouts.front-end.partials._cart',
    'products_floating_nav_partials' => '',
    'products_cart_details_partials' => 'web-views.cart._cart-details',
    'product_reviews_partials' => 'web-views.partials._product-reviews',
    'product_quick_view_partials' => 'web-views.partials._quick-view-data',
    'product_shipping_method_modal_view_partials' => 'layouts.front-end.partials.modal._buy-now-data',
    'modal_for_social_media_user_view' => 'layouts.front-end.partials.modal._social-media-user-modal',
    'cart_list' => 'web-views.cart.cart-list',
    'order_shipping' => 'web-views.checkout.shipping',
    'payment_details' => 'web-views.checkout.payment',
    'payment_failed' => 'web-views.checkout.payment-failed',
    'order_complete' => 'web-views.checkout.complete',
    'products_mobile_nav_partials' => 'layouts.front-end.partials._cart',
    'products_view_page' => 'web-views.products.view',
    'products__ajax_partials' => 'web-views.products._ajax-products',
    'all_brands' => 'web-views.products.brands',
    'contacts' => 'web-views.pages.contact-us',
    'flash_deals' => 'web-views.products.flash-deals',
    'shop_view_page' => 'web-views.seller-view.shopview',
    'faq' => 'web-views.pages.help-topics',
    'business_page' => 'web-views.pages.business_page',
    'recover_password' => 'web-views.customer-views.auth.recover-password',
    'otp_verification' => 'web-views.customer-views.auth.verify-otp',
    'reset_password' => 'web-views.customer-views.auth.reset-password',
    'wallet_account' => '',
    'product_search_result_for_compare_list' => '',
    'maintenance_mode' => 'web-views.maintenance-mode',
    'pay_offline_method_list_partials' => 'web-views.partials._pay-offline-method-list-field',
    'digital_product_order_otp_verify' => 'layouts.front-end.partials._digital-product-order-otp-verify',
    'digital_product_order_otp_verify_failed' => 'layouts.front-end.partials._digital-product-order-otp-verify-failed',
    'digital_product_download' => 'web-views.order.digital-product-download',
    'robots_meta_content_partials' => 'web-views.partials._robotsMetaContentData',
    'product_seo_meta_content_partials' => 'web-views.partials._productSEOMetaContentData',
    'analytics_script_partials' => 'layouts.front-end.partials._analytics-script',

    'frontend_blog_list' => 'web-views.blogs.index',
    'frontend_blog_details_view' => 'web-views.blogs.details',
    'frontend_app_blog_list' => 'web-views.blogs.app.index',
    'frontend_app_blog_details_view' => 'web-views.blogs.details',
    'frontend_popular_blogs_view' => 'web-views.blogs.index-popular',
    'blog_seo_meta_content_partials' => 'web-views.partials._blogSEOMetaContentData',


    'frontend_auction_homepage' => 'auction.web-views.auction',
    'frontend_auction_product_list' => 'auction.web-views.product._list',
    'frontend_auction_product_details' => 'auction.web-views.product._product-details',
    'frontend_auction_bid_list' => 'auction.web-views.product._bid-list',
    'frontend_auction_insight_content' => 'auction.web-views.product.details._insight-content',

    'frontend_auction_product_cart_option_container' => 'auction.web-views.product.details._product-cart-option-container',
    'frontend_auction_place_bid_modal_content' => 'auction.web-views.product.details._place-bid-modal-content',
    'frontend_auction_claim_checkout'          => 'auction.web-views.product.details._claim-checkout',
    'auction_pay_offline_method_list_partials' => 'auction.web-views.partials._pay-offline-method-list-field',

    'frontend_auction_bid_list_page' => 'auction.web-views.user-profile._bid-list-page',
    'frontend_auction_profile_product_details' => 'auction.web-views.user-profile._product-details',
    'frontend_auction_profile_auction_info' => 'auction.web-views.user-profile._product-details-auction-info',
    'frontend_auction_profile_product_info' => 'auction.web-views.user-profile._product-details-product-info',
    'frontend_auction_profile_payment_info' => 'auction.web-views.user-profile._product-details-payment-info',
    'frontend_auction_profile_billing_info' => 'auction.web-views.user-profile._product-details-billing-info',
    'frontend_auction_profile_shipping_address' => 'auction.web-views.user-profile._product-details-shipping-address',
    'frontend_auction_saved_list_page' => 'auction.web-views.user-profile._saved-product-list',

    'frontend_auction_create_product' => 'auction.web-views.user-profile._auction-create-product',
    'frontend_auction_update_product' => 'auction.web-views.user-profile._auction-update-product',
    'frontend_auction_recreate_product' => 'auction.web-views.user-profile._auction-recreate-product',
    'frontend_profile_auction_list' => 'auction.web-views.user-profile._auction-list',
    'frontend_profile_auction_pending_list' => 'auction.web-views.user-profile._pending-list',


    'frontend_auction_sales_report' => 'auction.web-views.user-profile._sale-reports',
    'frontend_auction_transaction_history' => 'auction.web-views.user-profile._transaction-history',
];
