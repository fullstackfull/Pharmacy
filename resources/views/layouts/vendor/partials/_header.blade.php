@php
    ...
    $vendor = Seller::with(['shop'])->where('id', auth('seller')->id())->first();
        ...
        <div class="navbar-nav-wrap">
            <div class="navbar-brand-wrapper d-none d-sm-block d-xl-none">
                <a class="navbar-brand" href="{{ route('vendor.dashboard.index') }}" aria-label="">
                    @if (isset($vendor?->shop))
                        <img class="navbar-brand-logo"
                             src="{{ getStorageImages(path: $vendor?->shop?->image_full_url ?? '', type: 'backend-logo') }}"
                             alt="{{ translate('logo') }}"
                             height="40">
                    ...
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a title="{{ translate('website_shop_view') }}"
                               ...
                               href="{{ !empty($vendor?->shop) ? route('vendor-shop', ['slug' => $vendor?->shop?->slug ?? '']) : 'javascript:;' }}"
                               target="_blank"
                               data-toggle="tooltip"
                    ...
                    <li class="nav-item mt-n2">
                        <div class="hs-unfold">
                            ...
                            <div class="topbar-text dropdown disable-autohide {{ $direction === 'rtl' ? 'ml-3' : 'm-1' }} text-capitalize">
                                <a class="topbar-link d-flex align-items-center" href="javascript:" data-toggle="dropdown">
                                    @if(!empty($lang))
                                        @foreach(json_decode($lang['value'] ?? '[]', true) as $data)
                                            @php
                                                $langFlagCode = $data['code'];
                                                if (\Illuminate\Support\Str::contains($data['code'], '-')) {
                                                    $langFlagCode = explode('-', $data['code'])[0];
                                                }
                                            @endphp
                                            @if($data['code'] == $local)
                                                <img width="20"
                                                     src="{{ dynamicAsset('public/assets/front-end/img/flags/' . strtolower($langFlagCode) . '.png') }}"
                                                     alt="{{ $data['name'] }}">
                                            @endif
                                        @endforeach
                                    @endif
        ...
        <div id="website_info" class="bg-secondary w-100 d-none">
            <div class="p-3">
                ...
                <div class="bg-white p-1 rounded mt-2">
                    <a title="{{ $vendor?->shop?->name }}" class="p-2 title-color"
                       href="{{ route('vendor-shop', ['slug' => $vendor?->shop?->slug ])}}" target="_blank" data-toggle="tooltip"
                       data-custom-class="header-icon-title">
