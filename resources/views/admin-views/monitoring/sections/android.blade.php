{{--
    ...
    sessions and crashes the app reports about itself — is _mobile-app, shared with iOS: the two
    ...
    What is drawn here and not there is the part that is genuinely Android's own, and it is drawn
    BEFORE the numbers rather than under them: every figure on this page is a subset of requests
    that a middleware decided was Android, and that decision is a declared header first and a
    user-agent guess second. The guess matches the word "android", which every mainstream browser
    on an Android handset also sends. A reader who does not know that will read "requests from the
    Android app" as a fact about the app; on a shop whose app sends no X-Platform it is closer to a
    fact about Android devices. So the rule is stated as a caveat above the cards and transcribed
    rule by rule below them, and the one number that would settle it is shown as the unconfigured
    reading it is.
    ...
    the exact change that would produce it.
...
<div class="mon-attention">
    <div class="mon-attention__item mon-attention__item--info">
        ...
        <span class="mon-attention__body">
            <strong>{{ translate('what_this_page_counts_as_the_android_app') }}</strong>
...
{{-- The measured page, shared with iOS. --}}
...
{{-- The selection rule itself, so the figures above can be checked rather than believed. Two of
     ...
     is which instead of leaving "Android" to mean whatever the reader assumes. --}}
