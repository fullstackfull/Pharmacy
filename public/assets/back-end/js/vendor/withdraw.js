...
$('.status-filter').on('change',function (){
    ...
    let baseUrl = $('.withdraw-export-btn').data('base-url');
    let url = new URL(baseUrl, window.location.origin);
    url.searchParams.set('status', status);
    $('.withdraw-export-btn').attr('href', url.toString());
...
$('.with_note').on('select2:open', function () {
    setTimeout(() => {
        $('.select2-results__options > .select2-custom-note').remove();
        const noteId = $(this).data('note-id');
        const noteContent = $(`#${noteId}`).html();
        $('.select2-results__options').each(function () {
            if (!$(this).hasClass('select2-results__options--nested')) {
                $(this).append(noteContent);
            }
        });
    }, 0);
});
...
$('.vendor-withdrawal-method-update').on('change', function () {

    const selectedOption = $(this).find('option:selected');
    const methodId = selectedOption.val();
    const methodType = selectedOption.data('type');
    const currentRequest = $(this)?.data('current-request');
    ...
    $.ajax({
        ...
        success: function (response) {
            $(container).empty().html(response?.htmlView);
        },
        error: function () {
