...
| بائعونا (Vendor slider) | **تختار البائعين** بالاسم أو اتركه فارغاً للأعلى تقييماً · أسلوب: بطاقات/شريط/مضغوط · إظهار التقييم وعدد المنتجات | جدول المتاجر — التقييم وعدد المنتجات من نفس مصدر صفحة `/vendors` |
| واجهة بائع (Vendor showcase) | **تختار متجراً** فيظهر غلافه وشعاره وتقييمه وزر «زيارة المتجر» ومنتجاته | المتجر ومنتجاته (المتجر الرئيسي `admin` والبائعون `seller` كلاهما مدعوم) |
...
> **مهم — عرض فلاش واحد فقط يكون «مُفعّلاً»:** لوحة التحكم تُطفئ باقي العروض عند تفعيل واحد.
> لذلك إن تركت قسمين على «الجاري حالياً» فلن يجدا سوى العرض نفسه. **اختر العرض بالاسم في كل
> قسم** ليعرض كل قسم عرضاً مختلفاً — والقسم يعرض ما اخترته حتى لو كان غير مُفعَّل، ويختفي فقط
> إذا كان العرض قد **انتهى** (لا نعرض عدّاداً ميتاً). وإن تركت قسمين تلقائيَّين فالثاني يتخطّى
> العرض الذي ظهر قبله بدل تكراره.
| عرض اليوم (Deal of the day) | العنوان (أو اسم العرض تلقائياً) · عدّاد حتى منتصف الليل · زر إضافة للسلة على البطاقة | Promotion → Deal of the day — المنتج الذي حدّدته أنت |
| العرض المميز (Featured deal) | rail أو grid · عدد المنتجات · زر إضافة للسلة | Promotion → Featured deal — منتجات الحملة الجارية بتواريخها |
| تصفية المخزون (Clearance) | rail أو grid · عدد المنتجات · زر إضافة للسلة | Promotion → Clearance sale — المنتجات المفعّلة فيها |
| الكوبونات (Coupon strip) | العنوان · عدد الكوبونات | Promotion → Coupons — الكوبونات السارية فقط (تاريخ ساري + مفعّلة + عامة)، وكل بطاقة تنسخ الكود بضغطة |
| إحصائيات المتجر (Store stats) | لكل عدّاد: **مصدر القيمة** (منتجات/ماركات/فئات/عملاء/طلبات/بائعين) أو **قيمة مكتوبة** + لاحقة (+، K) وأيقونة · بطاقات فاتحة أو لوحة داكنة · عدّ تصاعدي عند الظهور | عدّ حقيقي من جداول المتجر — لا أرقام مزيّنة |
| باقة (Bundle) | **تختار منتجات الطقم** + نسبة خصم الباقة · العنوان والوصف ونص الزر | كتالوج المنتجات — يحسب سعر الطقم قبل/بعد ونسبة التوفير، وزر واحد يضيفها كلها إلى السلة |
| تسوّق حسب الاهتمام | بلاطات كبيرة: صورة، عنوان، وصف، رابط، شفافية الطبقة، ارتفاع | ما تكتبه في المحرر — والرابط يقود إلى صفحة مفلترة جاهزة |
| القصص (Stories) | لكل قصة: غلاف أو فيديو mp4، عنوان، رابط ونص زر | ما ترفعه في المحرر — تُفتح بملء الشاشة مع تنقّل بالسحب ولوحة المفاتيح |
| من المدونة | عدد المقالات · زر «عرض الكل» | وحدة Blog — أحدث المقالات المنشورة (بلا مقالات لا يظهر القسم) |
| فروعنا | لكل فرع: الاسم، العنوان، ساعات العمل، الهاتف، رابط الخريطة | ما تكتبه في المحرر |
| عدّاد الشحن اليوم | **وقت الإغلاق** (24 ساعة) · شريط أو بطاقة · العنوان والوصف | ساعة الخادم — يعدّ حتى وقت الإغلاق ويختفي بعده بدل عرض عدّاد ميت |
| قبل / بعد | لكل زوج: صورة «قبل» وصورة «بعد» + عنوان وتعليق وارتفاع | ما ترفعه في المحرر — شريط يسحبه العميل، ويعمل بنفس الاتجاه في العربية والإنجليزية |
...
**أقسام جديدة تقرأ ما يعرفه متجرك فعلاً:**

| القسم | ماذا يعرض | مصدر البيانات |
|---|---|---|
| تبويبات المنتجات (Product tabs) | صف واحد من المنتجات مع تبويبات في أعلاه، **لكل تبويب مصدره** (وصل حديثاً / الأكثر مبيعاً / الأعلى تقييماً / فئة / ماركة / اختيار يدوي) | نفس مصادر سلايدر المنتجات — يوفّر ثلاثة أقسام وثلاث شاشات من الطول |
| واجهة ماركة (Brand showcase) | ماركة واحدة بشعارها ومنتجاتها وزر «عرض الكل» | جدول الماركات + منتجاتها |
| الأكثر بحثاً (Trending searches) | **الكلمات التي بحث عنها عملاؤك فعلاً** — لا قائمة تكتبها بيدك | تجميعة التحليلات (`analytics_daily`) بعد استبعاد الزواحف وموظفي المتجر |
| شوهد مؤخراً (Recently viewed) | المنتجات التي شاهدها **هذا الزائر بنفسه** | أحداث التحليلات مربوطة بكوكي الزائر وحده — لا يقرأ تاريخ غيره أبداً |
| حمّل التطبيق (Get the app) | أزرار المتجرين + **رمز QR** يفتح نفس الرابط | إعدادات الروابط العميقة — وبلا روابط مضبوطة لا يظهر القسم ويخبرك المحرر بذلك |
| تسوّق حسب السعر (Shop by price) | بلاطات نطاقات سعرية تقود إلى قائمة مفلترة | ما تكتبه في المحرر (من/إلى)، والرابط يفتح صفحة المنتجات مفلترة بالسعر |

**أكثر من طريقة عرض للقسم الواحد:** ٢٣ قسماً تملك الآن خيار «طريقة العرض»، بمجموع ٥٥ شكلاً —
مثلاً سلايدر المنتجات: شريط، شبكة، كاروسيل يُطلّ على البطاقة التالية، تسليط الضوء على الأول،
أو قائمة صف-لكل-منتج (الأنسب للصيدلية: اسم العبوة والماركة مقروءان بدل أن يُقصّا تحت صورة).
وشبكة الفئات: حلقات، بطاقات، بلاطات بالصور، أو شرائح مضغوطة. ويحرس ذلك اختبار
`tests/Feature/ThemeDisplayVariantTest.php`: أي «طريقة عرض» معروضة ولا تُغيّر شيئاً تُسقط الاختبارات.

باقي الأقسام: هيرو سلايدر، شبكة فئات، بانرات (grid/mosaic/split/strip/carousel)، نشرة بريدية، HTML مخصص، مسافة.

> **قسم بلا بيانات لا يظهر:** كل قسم يتغذّى من لوحة التحكم (عرض اليوم، العرض المميز، التصفية،
> الكوبونات، المدونة، الباقة، عدّاد الشحن) يُحسب **قبل** رسم إطاره — فإن لم تكن بياناته موجودة
> بعد لا يترك فراغاً مبطّناً في الصفحة، بل يختفي كلياً. وكذلك أقسام العناصر (القصص، الفروع،
> قبل/بعد): العنصر الذي لم تُكمل محتواه يُستبعد، والقسم الفارغ لا يظهر.
...
- **بطاقة المنتج احترافية:** تحت الصورة اسم الماركة بخط خفيف، ثم اسم المنتج بسطرين كاملين
  (بلا قصّ في منتصف الحرف — القصّ عند نهاية السطر مع «…»)، ثم نجوم التقييم وعدد المراجعات،
  ثم السعر بعد الخصم والسعر قبله مشطوباً، وشارة **نسبة الخصم** («‎-15%») تُحسب من الفرق بين
  السعرين فتظهر صحيحة سواء كان الخصم نسبة أو مبلغاً ثابتاً. وفي أعلى الصورة **قلب المفضلة**
  (نفس مفضّلة المتجر: يمتلئ عند الحفظ ويبقى ممتلئاً بعد إعادة التحميل، والزائر غير المسجَّل
  يُطلب منه تسجيل الدخول)، وشارة «غير متوفر» عند نفاد المخزون.
...
- **شارة «هذا القسم لن يظهر» على قائمة الأقسام نفسها:** بجانب اسم كل قسم تظهر علامة إن كان لن
  يُرسم على المتجر، مع سبب واحد واضح: **برتقالية** حين القرار قرارك (اختر فئة، أضف بطاقة، اربط
  متجر التطبيق) و**رمادية** حين لا شيء لديك بعد لعرضه (لا كوبون سارٍ، لا مقال منشور، انتهى وقت
  الشحن اليوم). القرار نفسه الذي يتخذه المتجر عند الرسم — كلاهما يسأل
  `app/Services/Theme/SectionReadiness.php` — فلا يمكن أن يختلف التنبيه عن الواقع.
