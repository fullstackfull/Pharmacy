...
| 2026-08-24 | C.1 | Returns reach the seller. Approving a refund now opens the return the marketplace already knew how to handle — idempotent on the refund, skipped for digital items — so the goods a refund implies stop being invisible. Five endpoints and an app screen follow one from authorised to received or refused; restocking is decided at receipt, not at authorisation, and goes through the stock ledger as a `return` movement. The detail view shows the refund's ledger lines including the commission credit sellers never see. Fixed: `receive()` stamped every receipt 'admin', and a refund id belonging to another seller fataled into a 500 instead of a not-found. 11 backend tests (three mutations verified), 6 app contract tests. |
| 2026-08-24 | B.3 | The stock ledger reaches the seller. Five endpoints — shop state, movement log, reasoned correction, warehouses, expiring batches — plus a Stock tab on every product page and an Inventory entry in the menu, both rendering the same view narrowed differently. A correction goes through `InventoryService::adjust()`: locked row, refused below zero, movement written with a required reason. `InventoryService::recent()` gains a seller filter; without it that method is an admin view of every seller's history. Warehouses and batches answer honestly where the marketplace does not run them, so an unused module stays hidden rather than appearing empty. 14 backend tests (three mutations verified), 7 app contract tests. |
| 2026-08-24 | B.2 | Per-order fee breakdown and order timeline, on the API, in the app and on the vendor panel — all three from the same two services, so none of them can quote a different margin. The breakdown reads the per-line commission ledger where it exists (naming the rule that charged), the older transaction row otherwise, and answers `not_recorded` with a null seller-receives figure when neither does: a number a seller reads as "what I will be paid" may only come from the record that pays them. Ledger lines show when a pending earning becomes withdrawable, which nothing has ever shown per order. The timeline is built only from rows that exist — status changes with who caused them, courier assignment, date changes, pick/pack/ship, edits, refunds. The processing countdown moved into `SlaService`, which the Action Center producer now reads too. 12 backend tests (three mutations verified), 6 app contract tests. |
| 2026-08-24 | B.1 | The Action Center, in the app. One list, worst first, read from the insight store the backend has kept since A.1 and had no interface. Severity filter with live counts, dismissal (refused on critical), and a home card that renders nothing at all when nothing is waiting. Each entry opens the record it is about: an order goes to its detail screen, a product loads and opens the editor — the editor reads real fields, so a stub built from an id would open a form disagreeing with the record. A rejected listing shows the moderator's own reason codes and note first, which the app had never surfaced; where none was recorded it says so. 11 contract tests pinned to live payloads. |
| 2026-08-24 | A.3 | Bulk price and bulk stock, each with a receipt. `seller_bulk_jobs` records what was asked for before any of it runs, and the job updates it as it goes; `partial` is a first-class outcome, so a run that changed 380 of 400 says which 20 it did not and why, downloadable as a CSV. Ownership is proved by the WHERE clause, never by the ids in the request, and an id belonging to another seller is refused as not found. Stock goes through the ledger rather than the column, so it cannot be driven negative and every movement is explained; a variant product is refused rather than guessed at. Fixed two unscoped stock writes on the way past (`updateProductQuantity`, `updateRestockQuantity`) that let any seller set any product's stock in the marketplace. A scheduled sweep runs any job the queue never picked up, so the feature does not silently depend on a worker being installed. App side: a product picker, the two operations, the receipt with its reasons, and the CSV. 21 tests. |
